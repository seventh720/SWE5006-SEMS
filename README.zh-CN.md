...
组织者统计通过 `GET /api/v1/organizer/events/summary` 获取，需 `ORGANIZER` 或 `ADMIN`，只统计 JWT 当前用户的数据，不受列表分页影响。响应包含 `drafts`、`published`、`cancelled`、`upcoming`。管理员账号概况复用现有用户列表接口，角色统计可能重叠，因为账号支持多角色。
